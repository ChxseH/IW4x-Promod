...
init() {
  ...
  # /
    if (destructibles_enabled) find_destructibles();
  deletables = GetEntArray("delete_on_load", "targetname");
  foreach(ent in deletables) ent Delete();
  /# SetDevDvarIfUninitialized( "scr_destructible_warning", "1" );
  if (GetDvarInt("scr_destructible_warning", 1) == 1) thread warn_about_old_destructible();
...
warn_about_old_destructible() {
  ...
  if (destructibles.size != 0) {
    ...
    PrintLn("^1###################################################^0");
    PrintLn("^1###################################################^0");
    foreach(vehicle in destructibles) PrintLn("^1Destructible vehicle at ( " + vehicle.origin + " ) uses an old prefab. Update it to use a prefab located in prefabs\destructible\^0");
    PrintLn("^1###################################################^0");
    PrintLn("^1###################################################^0");
    PrintLn("^1###################################################^0");
...
find_destructibles() {
  ...
  foreach(vehicle in vehicles) vehicle setup_destructibles();
  destructible_toy = GetEntArray("destructible_toy", "targetname");
  foreach(toy in destructible_toy) toy setup_destructibles();
  /# total = 0;
...
setup_destructibles(cached) {
  if (!isdefined(cached)) cached = false;
  ...
  if (self.destuctableInfo < 0) return;
  /# if ( !isdefined( level.created_destructibles[ self.destructible_type ] ) ) level.created_destructibles[ self.destructible_type ] = [];
  ...
  # /
    if (!cached) precache_destructibles();
  add_destructible_fx();
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts)) {
    ...
    for (i = 0; i < level.destructible_type[self.destuctableInfo].parts.size; i++) {
      ...
      if (IsDefined(level.destructible_type[self.destuctableInfo].parts[i][0].v["health"])) self.destructible_parts[i].v["health"] = level.destructible_type[self.destuctableInfo].parts[i][0].v["health"];
      ...
      while (IsDefined(level.destructible_type[self.destuctableInfo].parts[i][stateIndex])) {
        ...
        if (IsDefined(stateTagName) && stateTagName != tagName) {
          ...
          if (self.modeldummyon) self.modeldummy hideapart(stateTagName);
        }
        stateIndex++;
      }
    }
  }
  if (IsDefined(self.target)) thread destructible_handles_collision_brushes();
  if (self.code_classname != "script_vehicle") self SetCanDamage(true);
  if (isSP()) self thread connectTraverses();
...
destructible_create(type, tagName, health, validAttackers, validDamageZone, validDamageCause) {
  ...
  if (!isdefined(level.destructible_type)) level.destructible_type = [];
...
destructible_part(tagName, modelName, health, validAttackers, validDamageZone, validDamageCause, alsoDamageParent, physicsOnExplosion, grenadeImpactDeath, receiveDamageFromParent) {
  ...
  Assert(partIndex > 0);
  stateIndex = 0;
  if (isSubStr(tagName, "glass")) health = 5;
...
destructible_state(tagName, modelName, health, validAttackers, validDamageZone, validDamageCause, grenadeImpactDeath, splashRotation) {
  ...
  if (!isdefined(tagName) && partIndex == 0) tagName = level.destructible_type[destructibleIndex].parts[partIndex][0].v["tagName"];
  if (isSubStr(tagName, "glass")) health = 5;
...
destructible_fx(tagName, fxName, useTagAngles, damageType, groupNum, fxCost) {
  ...
  if (!isdefined(useTagAngles)) useTagAngles = true;
  if (!isdefined(groupNum)) groupNum = 0;
  if (!isdefined(fxCost)) fxCost = 0;
  destructibleIndex = (level.destructible_type.size - 1);
  partIndex = (level.destructible_type[destructibleIndex].parts.size - 1);
  ...
  if (IsDefined(level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["fx_filename"]))
    if (IsDefined(level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["fx_filename"][groupNum])) fx_size = level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["fx_filename"][groupNum].size;
  if (IsDefined(damageType)) level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["fx_valid_damagetype"][groupNum][fx_size] = damageType;
...
destructible_loopfx(tagName, fxName, loopRate, fxCost) {
  ...
  Assert(IsDefined(level.destructible_type[destructibleIndex].parts[partIndex][stateIndex]));
  fx_size = 0;
  if (IsDefined(level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["loopfx_filename"])) fx_size = level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["loopfx_filename"].size;
...
destructible_anim(animName, animTree, animType, vehicle_exclude, groupNum, mpAnim, maxStartDelay, animRateMin, animRateMax) {
  if (!isdefined(vehicle_exclude)) vehicle_exclude = false;
...
random_dynamic_attachment(tagName, attachment_1, attachment_2, clipToRemove) {
  ...
  if (!isdefined(attachment_2)) attachment_2 = "";
  destructibleIndex = (level.destructible_type.size - 1);
  partIndex = (level.destructible_type[destructibleIndex].parts.size - 1);
  stateIndex = 0;
  Assert(IsDefined(level.destructible_type));
...
destructible_explode(force_min, force_max, rangeSP, rangeMP, mindamage, maxdamage, continueDamage, originOffset, earthQuakeScale, earthQuakeRadius) {
  ...
  Assert(IsDefined(level.destructible_type[destructibleIndex].parts[partIndex]));
  Assert(IsDefined(level.destructible_type[destructibleIndex].parts[partIndex][stateIndex]));
  if (isSP()) level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["explode_range"] = rangeSP;
  else level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["explode_range"] = rangeMP;
...
destructible_info(partIndex, stateIndex, tagName, modelName, health, validAttackers, validDamageZone, validDamageCause, alsoDamageParent, physicsOnExplosion, grenadeImpactDeath, splashRotation, receiveDamageFromParent) {
  ...
  if (IsDefined(modelName)) modelName = ToLower(modelName);
  destructibleIndex = (level.destructible_type.size - 1);
...
precache_destructibles() {
  if (!isdefined(level.destructible_type[self.destuctableInfo].parts)) return;
  for (i = 0; i < level.destructible_type[self.destuctableInfo].parts.size; i++) {
    for (j = 0; j < level.destructible_type[self.destuctableInfo].parts[i].size; j++) {
      if (level.destructible_type[self.destuctableInfo].parts[i].size <= j) continue;
      ...
      if (IsDefined(level.destructible_type[self.destuctableInfo].parts[i][j].v["animation"])) {
        ...
        foreach(group in animGroups) {
          if (IsDefined(group["mpAnim"])) noself_func("precacheMpAnim", group["mpAnim"]);
        }
      }
  ...
  for (i = 0; i < level.destructible_type[self.destuctableInfo].parts.size; i++) {
    for (j = 0; j < level.destructible_type[self.destuctableInfo].parts[i].size; j++) {
      ...
      if (IsDefined(part.v["fx_filename"])) {
        for (g = 0; g < part.v["fx_filename"].size; g++) {
          ...
          if (IsDefined(fx_filenames)) {
            if (IsDefined(part.v["fx"]) && IsDefined(part.v["fx"][g]) && part.v["fx"][g].size == fx_filenames.size) continue;
      ...
      if (IsDefined(loopfx_filenames)) {
        if (IsDefined(part.v["loopfx"]) && part.v["loopfx"].size == loopfx_filenames.size) continue;
...
canDamageDestructible(testDestructible) {
  foreach(destructible in self.destructibles) {
    if (destructible == testDestructible) return true;
  }
  return false;
}
destructible_think() {
  ...
  for (;;) {
    ...
    if (!isdefined(damage)) continue;
    if (IsDefined(attacker) && IsDefined(attacker.type) && attacker.type == "soft_landing" && !attacker canDamageDestructible(self)) continue;
    if (isDefined(level.strat_over) && !level.strat_over) continue;
    if (isSP()) damage *= SP_DAMAGE_BIAS;
    else damage *= MP_DAMAGE_BIAS;
    if (damage <= 0) continue;
    if (IsDefined(attacker) && IsPlayer(attacker)) self.damageOwner = attacker;
    type = getDamageType(type);
    Assert(IsDefined(type));
    if (is_shotgun_damage(attacker, type)) {
      if (isSP()) damage *= SP_SHOTGUN_BIAS;
      else damage *= MP_SHOTGUN_BIAS;
    }
    /# if ( GetDvarInt( "debug_destructibles" , 0 ) == 1 ) { Print3d( point, ".", ( 1, 1, 1 ), 1.0, 0.5, 100 );
    if (IsDefined(damage)) IPrintLn("damage amount: " + damage);
    if (IsDefined(modelName)) IPrintLn("hit model: " + modelName);
    if (IsDefined(tagName)) IPrintLn("hit model tag: " + tagName);
    else IPrintLn("hit model tag: ");
  }
  # /
  ...
  if (IsDefined(tagName) && tagName == "") {
    if (IsDefined(partName) && partName != "" && partName != "tag_body" && partName != "body_animate_jnt") tagName = partName;
    else tagName = undefined;
    baseModelTag = level.destructible_type[self.destuctableInfo].parts[0][0].v["tagName"];
    if (IsDefined(baseModelTag) && IsDefined(partName) && (baseModelTag == partName)) tagName = undefined;
  }
  prof_end("_destructible");
  if (type == "splash") {
    /# if ( GetDvarInt( "debug_destructibles" , 0 ) == 1 ) IPrintLn( "type = splash" );
    # /
      if (IsDefined(level.destructible_type[self.destuctableInfo].parts[0][0].v["splash_damage_scaler"])) damage *= level.destructible_type[self.destuctableInfo].parts[0][0].v["splash_damage_scaler"];
      else {
        if (isSP()) damage *= SP_EXPLOSIVE_DAMAGE_BIAS;
        else damage *= MP_EXPLOSIVE_DAMAGE_BIAS;
...
is_shotgun_damage(attacker, type) {
  ...
  if (!isdefined(attacker)) return false;
  currentWeapon = undefined;
  if (IsPlayer(attacker)) {
    currentweapon = attacker getCurrentWeapon();
  }
  if (!isdefined(currentweapon)) return false;
  class = weaponClass(currentweapon);
  if (isdefined(class) && class == "spread") return true;
  return false;
}
getPartAndStateIndex(modelName, tagName) {
  ...
  for (i = 0; i < level.destructible_type[self.destuctableInfo].parts.size; i++) {
    ...
    if (level.destructible_type[self.destuctableInfo].parts[i].size <= stateIndex) continue;
    if (!isdefined(tagName)) continue;
...
destructible_update_part(damage, modelName, tagName, point, direction_vec, attacker, damageType, partInfo) {
  if (!isdefined(self.destructible_parts)) return;
  if (self.destructible_parts.size == 0) return;
  prof_begin("_destructible");
  ...
  for (;;) {
    ...
    if (!isdefined(level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex])) break;
    ...
    if (getDamageType(damageType) != "splash") {
      foreach(part in level.destructible_type[self.destuctableInfo].parts) {
        if (!isdefined(part[0].v["receiveDamageFromParent"])) continue;
        if (!isdefined(part[0].v["tagName"])) continue;
    ...
    if (!isdefined(level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex].v["health"])) break;
    if (!isdefined(self.destructible_parts[partIndex].v["health"])) break;
    if (updateHealthValue) self.destructible_parts[partIndex].v["health"] = level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex].v["health"];
    updateHealthValue = false;
    /# if ( GetDvarInt( "debug_destructibles" , 0 ) == 1 ) { IPrintLn( "stateindex: " + stateIndex );
    IPrintLn("damage: " + damage);
    IPrintLn("health( before ): " + self.destructible_parts[partIndex].v["health"]);
  }
  # /
    if ((IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex].v["grenadeImpactDeath"])) && (damageType == "impact")) damage = 100000000;
    ...
    if (validDamageCause) {
      if (IsDefined(attacker)) {
        if (IsPlayer(attacker)) {
          ...
          if (attacker != self) self.non_player_damage += damage;
        }
      }
      if (IsDefined(damageType)) {
        if (damageType == "melee" || damageType == "impact") damage = 100000;
      }
      self.destructible_parts[partIndex].v["health"] -= damage;
    }
  }
  /# if ( GetDvarInt( "debug_destructibles" , 0 ) == 1 ) IPrintLn( "health( after ): " + self.destructible_parts[ partIndex ].v[ "health" ] );
  # /
  ...
  action_v = undefined;
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex])) action_v = level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex].v;
  state_v = undefined;
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex])) state_v = level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex].v;
  if (!isdefined(level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex])) {
    prof_end("_destructible");
    return;
  }
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex].v["explode"])) self.exploding = true;
  if (IsDefined(self.loopingSoundStopNotifies) && IsDefined(self.loopingSoundStopNotifies[toString(partIndex)])) {
    for (i = 0; i < self.loopingSoundStopNotifies[toString(partIndex)].size; i++) {
      ...
      if (isSP() && self.modeldummyon) self.modeldummy notify(self.loopingSoundStopNotifies[toString(partIndex)][i]);
    }
  ...
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex])) {
    if (partIndex == 0) {
      ...
      if (IsDefined(newModel) && newModel != self.model) {
        ...
        if (isSP() && self.modeldummyon) self.modeldummy SetModel(newModel);
        destructible_splash_rotatation(state_v);
      }
    } else {
      self hideapart(tagName);
      if (isSP() && self.modeldummyon) self.modeldummy hideapart(tagName);
      tagName = state_v["tagName"];
      if (IsDefined(tagName)) {
        self showapart(tagName);
        if (isSP() && self.modeldummyon) self.modeldummy showapart(tagName);
      }
    }
  }
  eModel = get_dummy();
  if (IsDefined(self.exploding)) self clear_anims(eModel);
  ...
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex].v["loopfx"])) {
    ...
    if (loopfx_size > 0) self notify("FX_State_Change" + partIndex);
  ...
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex].v["loopsound"])) {
    for (i = 0; i < level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex].v["loopsound"].size; i++) {
      ...
      if (validSoundCause) {
        ...
        if (!isdefined(self.loopingSoundStopNotifies)) self.loopingSoundStopNotifies = [];
        if (!isdefined(self.loopingSoundStopNotifies[toString(partIndex)])) self.loopingSoundStopNotifies[toString(partIndex)] = [];
  ...
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex].v["physics"])) {
    for (i = 0; i < level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex].v["physics"].size; i++) {
      ...
      if (IsDefined(physVelocity)) {
        ...
        if (IsDefined(physTagName)) physTagAngles = self GetTagAngles(physTagName);
        else if (IsDefined(tagName)) physTagAngles = self GetTagAngles(tagName);
        Assert(IsDefined(physTagAngles));
        physTagOrigin = undefined;
        if (IsDefined(physTagName)) physTagOrigin = self GetTagOrigin(physTagName);
        else if (IsDefined(tagName)) physTagOrigin = self GetTagOrigin(tagName);
        ...
        /# if ( GetDvarInt( "debug_destructibles" , 0 ) == 1 ) { thread draw_line_for_time( physTagOrigin, physTagOrigin + initial_velocity, 1, 1, 1, 5.0 );
      }
      # /
    } else {
    ...
    if (IsDefined(physTagName)) {
      ...
      for (j = 0; j < level.destructible_type[self.destuctableInfo].parts.size; j++) {
        if (!isdefined(level.destructible_type[self.destuctableInfo].parts[j][0].v["tagName"])) continue;
        if (level.destructible_type[self.destuctableInfo].parts[j][0].v["tagName"] != physTagName) continue;
        physPartIndex = j;
        break;
      }
      if (IsDefined(physTagOrigin)) self thread physics_launch(physPartIndex, 0, physTagOrigin, initial_velocity);
      else self thread physics_launch(physPartIndex, 0, point, initial_velocity);
    } else {
      if (IsDefined(physTagOrigin)) self thread physics_launch(partIndex, actionStateIndex, physTagOrigin, initial_velocity);
      else self thread physics_launch(partIndex, actionStateIndex, point, initial_velocity);
      prof_end("_destructible");
      return;
    }
  }
...
destructible_splash_rotatation(v) {
  ...
  if (!isdefined(model_rotate_to)) return;
  if (!isdefined(model_rotation)) return;
  if (!model_rotation) return;
...
damage_not(damageType) {
  ...
  new_string = "";
  foreach(idx, tok in toks) damages_tok = array_remove(damages_tok, tok);
  foreach(damages in damages_tok) new_string += damages + " ";
  return new_string;
}
destructible_splash_damage(damage, point, direction_vec, attacker, damageType) {
  if (damage <= 0) return;
  if (IsDefined(self.exploded)) return;
  if (!isdefined(level.destructible_type[self.destuctableInfo].parts)) return;
  damagedParts = self getAllActiveParts(direction_vec);
  if (damagedParts.size <= 0) return;
  ...
  foreach(part in damagedParts) {
    ...
    if (IsDefined(self.exploded)) continue;
    /# if ( GetDvarInt( "debug_destructibles" , 0 ) == 1 ) { if ( IsDefined( part.v[ "tagName" ] ) ) Print3d( self GetTagOrigin( part.v[ "tagName" ] ), damageAmount, ( 1, 1, 1 ), 1.0, 0.5, 200 );
  }
...
getAllActiveParts(direction_vec) {
  ...
  if (!isdefined(level.destructible_type[self.destuctableInfo].parts)) return activeParts;
  prof_begin("_destructible");
  for (i = 0; i < level.destructible_type[self.destuctableInfo].parts.size; i++) {
    partIndex = i;
    ...
    if (!isdefined(level.destructible_type[self.destuctableInfo].parts[partIndex][currentState])) continue;
...
getLowestPartDistance(partList) {
  ...
  foreach(part in partList) {
    ...
    d = part.v["distance"];
    if (!isdefined(closestDist)) closestDist = d;
...
isValidSoundCause(soundCauseVar, action_v, soundIndex, damageType, groupNum) {
  if (isdefined(groupNum)) soundCause = action_v[soundCauseVar][groupNum][soundIndex];
  else soundCause = action_v[soundCauseVar][soundIndex];
  if (!isdefined(soundCause)) return true;
  if (soundCause == damageType) return true;
  return false;
}
isAttackerValid(partIndex, stateIndex, attacker) {
  if (IsDefined(self.forceExploding)) return true;
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex].v["explode"])) {
    if (IsDefined(self.dontAllowExplode)) return false;
  }
  if (!isdefined(attacker)) return true;
  if (attacker == self) return true;
  sType = level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex].v["validAttackers"];
  if (!isdefined(sType)) return true;
  if (sType == "no_player") {
    if (!isplayer(attacker)) return true;
    if (!isdefined(attacker.damageIsFromPlayer)) return true;
    if (attacker.damageIsFromPlayer == false) return true;
  } else if (sType == "player_only") {
    if (IsPlayer(attacker)) return true;
    if (IsDefined(attacker.damageIsFromPlayer) && attacker.damageIsFromPlayer) return true;
  } else if (sType == "no_ai" && IsDefined(level.isAIfunc)) {
    if (![
        [level.isAIfunc]
      ](attacker)) return true;
  } else if (sType == "ai_only" && IsDefined(level.isAIfunc)) {
    if ([
        [level.isAIfunc]
      ](attacker)) return true;
  } else {
...
isValidDamageCause(partIndex, stateIndex, damageType) {
  if (!isdefined(damageType)) return true;
  godModeAllowed = level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex].v["godModeAllowed"];
  if (godModeAllowed && ((IsDefined(self.godmode) && self.godmode) || (IsDefined(self.script_bulletshield) && self.script_bulletshield) && damageType == "bullet")) return false;
  validType = level.destructible_type[self.destuctableInfo].parts[partIndex][stateIndex].v["validDamageCause"];
  if (!isdefined(validType)) return true;
  if ((validType == "splash") && damageType != "splash") return false;
  if ((validType == "no_melee") && damageType == "melee" || damageType == "impact") return false;
  return true;
}
getDamageType(type) {
...
health_drain(amount, interval, partIndex, modelName, tagName, badplaceRadius, badplaceTeam) {
  ...
  if (IsDefined(level.disable_destructible_bad_places) && level.disable_destructible_bad_places) badplaceRadius = undefined;
  if (IsDefined(badplaceRadius) && IsDefined(badplaceTeam) && isSP()) {
    ...
    if (!isdefined(self.disableBadPlace)) {
      ...
      if (badplaceTeam == "both") call[[level.badplace_cylinder_func]](uniqueName, 0, self.origin, badplaceRadius, 128, "allies", "bad_guys");
      else call[[level.badplace_cylinder_func]](uniqueName, 0, self.origin, badplaceRadius, 128, badplaceTeam);
      self thread badplace_remove(uniqueName);
    }
  }
  while (isdefined(self) && self.destructible_parts[partIndex].v["health"] > 0) {
    /# if ( GetDvarInt( "debug_destructibles" , 0 ) == 1 ) { IPrintLn( "health before damage: " + self.destructible_parts[ partIndex ].v[ "health" ] );
...
physics_launch(partIndex, stateIndex, point, initial_velocity) {
  ...
  self hideapart(tagName);
  /# if ( GetDvarInt( "destructibles_enable_physics", 1 ) == 0 ) return;
  # /
    if (level.destructibleSpawnedEnts.size >= level.destructibleSpawnedEntsLimit) physics_object_remove(level.destructibleSpawnedEnts[0]);
...
physics_object_remove(ent) {
  ...
  for (i = 0; i < level.destructibleSpawnedEnts.size; i++) {
    if (level.destructibleSpawnedEnts[i] == ent) continue;
...
explode(partIndex, force_min, force_max, range, mindamage, maxdamage, continueDamage, originOffset, earthQuakeScale, earthQuakeRadius, attacker) {
  ...
  if (!isdefined(originOffset)) originOffset = 80;
  ...
  if (self.code_classname == "script_vehicle") self notify("death", attacker, self.damage_type);
  if (isSP()) self thread disconnectTraverses();
  wait 0.05;
  currentState = self.destructible_parts[partIndex].v["currentState"];
  Assert(IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex]));
  tagName = undefined;
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][currentState])) tagName = level.destructible_type[self.destuctableInfo].parts[partIndex][currentState].v["tagName"];
  if (IsDefined(tagName)) explosionOrigin = self GetTagOrigin(tagName);
  else explosionOrigin = self.origin;
  ...
  if (IsDefined(level.destructible_type[self.destuctableInfo].parts)) {
    for (i = (level.destructible_type[self.destuctableInfo].parts.size - 1); i >= 0; i--) {
      ...
      stateIndex = self.destructible_parts[i].v["currentState"];
      if (stateIndex >= level.destructible_type[self.destuctableInfo].parts[i].size) stateIndex = level.destructible_type[self.destuctableInfo].parts[i].size - 1;
      ...
      if (!isdefined(modelName)) continue;
      if (!isdefined(tagName)) continue;
  ...
  if (stopTakingDamage) self notify("stop_taking_damage");
  wait 0.05;
  ...
  if (isSP()) {
    if (level.gameskill == 0 && !self player_touching_post_clip()) self RadiusDamage(damageLocation, range, maxdamage, mindamage, self, "MOD_RIFLE_BULLET");
    else self RadiusDamage(damageLocation, range, maxdamage, mindamage, self);
  ...
  if (IsDefined(earthQuakeScale) && IsDefined(earthQuakeRadius)) Earthquake(earthQuakeScale, 2.0, damageLocation, earthQuakeRadius);
  /# if ( GetDvarInt( "destructibles_show_radiusdamage" ) == 1 ) thread debug_radiusdamage_circle( damageLocation, range, maxdamage, mindamage );
...
cleanupVars() {
  ...
  if (isdefined(self.waiting_for_queue)) self waittill("queue_processed");
  if (!isdefined(self)) return;
...
get_traverse_disconnect_brush() {
  if (!isdefined(self.target)) return undefined;
  targets = GetEntArray(self.target, "targetname");
  foreach(target in targets) {
    if (IsSpawner(target)) continue;
    if (IsDefined(target.script_destruct_collision)) continue;
    if (target.code_classname == "light") continue;
    if (!target.spawnflags & 1) continue;
    return target;
  }
}
hideapart(tagName) {
...
get_dummy() {
  if (!isSP()) return self;
  if (self.modeldummyon) eModel = self.modeldummy;
  else eModel = self;
  return eModel;
}
play_loop_sound_on_destructible(alias, tag) {
  ...
  if (IsDefined(tag)) org.origin = eModel GetTagOrigin(tag);
  else org.origin = eModel.origin;
...
notifyDamageAfterFrame(damage, attacker, direction_vec, point, damageType, modelName, tagName) {
  if (IsDefined(level.notifyDamageAfterFrame)) return;
...
should_do_car_alarm() {
  if (level.currentCarAlarms >= MAX_SIMULTANEOUS_CAR_ALARMS) return false;
  timeElapsed = undefined;
  if (!isdefined(level.lastCarAlarmTime)) {
    if (cointoss()) return true;
    timeElapsed = GetTime() - level.commonStartTime;
  } else {
    timeElapsed = GetTime() - level.lastCarAlarmTime;
  }
  Assert(IsDefined(timeElapsed));
  if (level.currentCarAlarms == 0 && timeElapsed >= NO_CAR_ALARM_MAX_ELAPSED_TIME) return true;
  if (RandomInt(100) <= 33) return true;
  return false;
}
do_random_dynamic_attachment(tagName, attach_model_1, attach_model_2, clipToRemove) {
  ...
  if (isSP()) {
    ...
    if (IsDefined(attach_model_2) && attach_model_2 != "") self Attach(attach_model_2, tagName, false);
  } else {
  ...
  if (isSP()) {
    ...
    if (IsDefined(attach_model_2) && attach_model_2 != "") spawnedModels[1] SetModel(attach_model_2 + DESTROYED_ATTACHMENT_SUFFIX);
  }
}
...
collision_brush_pre_explosion(clip) {
  waittillframeend;
  if (isSP()) clip call[[level.disconnectPathsFunction]]();
  self waittill("exploded");
  if (isSP()) clip call[[level.connectPathsFunction]]();
  clip Delete();
}
...
destructible_get_my_breakable_light(range) {
  ...
  if (!isdefined(the_light)) return;
  self.breakable_light = the_light;
}
break_nearest_light(range) {
  if (!isdefined(self.breakable_light)) return;
...
debug_circle_drawlines(circlepoints, duration, color, center) {
  ...
  for (i = 0; i < circlepoints.size; i++) {
    ...
    if (i + 1 >= circlepoints.size) end = circlepoints[0];
    else end = circlepoints[i + 1];
...
is_valid_damagetype(damageType, v, idx, groupNum) {
  ...
  if (IsDefined(v["fx_valid_damagetype"])) valid_damagetype = v["fx_valid_damagetype"][groupNum][idx];
  if (!isdefined(valid_damagetype)) return true;
  return IsSubStr(valid_damagetype, damageType);
}
destructible_sound_think(action_v, eModel, damageType, groupNum) {
  if (isdefined(self.exploded)) return undefined;
  if (!isDefined(action_v["sound"])) return undefined;
  if (!isdefined(groupNum)) groupNum = 0;
...
destructible_fx_think(action_v, eModel, damageType, partIndex, groupNum) {
  if (!isdefined(action_v["fx"])) return undefined;
  if (!isdefined(groupNum)) groupNum = randomInt(action_v["fx_filename"].size);
  ...
  for (idx = 0; idx < fx_size; idx++) {
    if (!is_valid_damagetype(damageType, action_v, idx, groupNum)) continue;
...
destructible_animation_think(action_v, eModel, damageType, partIndex) {
  if (IsDefined(self.exploded)) return undefined;
  if (!isdefined(action_v["animation"])) return undefined;
  ...
  if (!isdefined(animRateMin)) animRateMin = 1.0;
  if (!isdefined(animRateMax)) animRateMax = 1.0;
  if (animRateMin == animRateMax) animRate = animRateMin;
  else animRate = RandomFloatRange(animRateMin, animRateMax);
  vehicle_dodge_part_animation = array["vehicle_exclude_anim"];
  if (self.code_classname == "script_vehicle" && vehicle_dodge_part_animation) return undefined;
  eModel self_func("useanimtree", animTree);
  animType = array["animType"];
  if (!isdefined(self.animsApplied)) self.animsApplied = [];
  self.animsApplied[self.animsApplied.size] = animName;
  if (IsDefined(self.exploding)) self clear_anims(eModel);
  if (IsDefined(maxStartDelay) && maxStartDelay > 0) wait RandomFloat(maxStartDelay);
  if (!isSP()) {
    if (IsDefined(mpAnim)) self self_func("scriptModelPlayAnim", mpAnim);
    return groupNum;
  }
  if (animType == "setanim") {
...
clear_anims(eModel) {
  if (IsDefined(self.animsApplied)) {
    foreach(animation in self.animsApplied) {
      if (isSP()) eModel self_func("clearanim", animation, 0);
      else eModel self_func("scriptModelClearAnim");
    }
  }
}
init_destroyed_count() {
  ...
  if (isSP()) level.maxDestructions = 20;
  else level.maxDestructions = 2;
}
...
add_destructible_to_frame_queue(destructible, partInfo, damage) {
  ...
  if (partInfo.v["distance"] < level.destructibleFrameQueue[entNum].nearDistance) level.destructibleFrameQueue[entNum].nearDistance = partInfo.v["distance"];
...
handle_destructible_frame_queue() {
  ...
  for (i = 0; i < sortedQueue.size; i++) {
    if (get_destroyed_count() < get_max_destroyed_count()) {
      if (sortedQueue[i].fxCost) thread add_to_destroyed_count();
...
sort_destructible_frame_queue(unsortedQueue) {
  sortedQueue = [];
  foreach(destructibleInfo in unsortedQueue) sortedQueue[sortedQueue.size] = destructibleInfo;
  for (i = 1; i < sortedQueue.size; i++) {
    queueStruct = sortedQueue[i];
    for (j = i - 1; j >= 0 && get_better_destructible(queueStruct, sortedQueue[j]) == queueStruct; j--) sortedQueue[j + 1] = sortedQueue[j];
    sortedQueue[j + 1] = queueStruct;
  }
  return sortedQueue;
}
get_better_destructible(destructibleInfo1, destructibleInfo2) {
  if (destructibleInfo1.totalDamage > destructibleInfo2.totalDamage) return destructibleInfo1;
  else return destructibleInfo2;
}
get_part_FX_cost_for_action_state(partIndex, actionStateIndex) {
  fxCost = 0;
  if (!isDefined(level.destructible_type[self.destuctableInfo].parts[partIndex][actionStateIndex])) return fxCost;
  ...
  if (IsDefined(action_v["fx"])) {
    foreach(fxCostObj in action_v["fx_cost"]) {
      foreach(fxCostVal in fxCostObj) fxCost += fxCostVal;
    }
  }
  return fxCost;
}